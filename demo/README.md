...
3. Predict different labels in a long video from web by using a TSN model on cpu, with 3 frames for input steps (that is, random sample one from each 3 frames)
   and outputting result labels with score higher than 0.2, loading checkpoint from url.
    ...
    ```
python demo/long_video_demo.py /media/h/Elements/work_dirs/tsm/tsm.py /media/h/Elements/work_dirs/tsm/latest.pth /media/h/My Passport/合笼视频/m13交配/22-03-17_28/ch06_20220317194301.mp4 tools/data/ucf101/pangolinmap.txt ./resultpangolin.json pangolin.mp4 --input-step 3 --device gpu --threshold 0.2
